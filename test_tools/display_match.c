...
{
    int no_matched =
        sscanf(line_buffer, "%s %*d %s %d %*d %s * %*d %*d %s %*s\n", read_name_buffer,
...
static void display_match(const char* pattern,
                          const char* cigar,
                          ...
                          struct fasta_records* ref_genome)
{
    ...
    const char* match_end = cigar_alignment(
        cigar, pattern, matched_seq, (char*)&pattern_buffer, (char*)&match_buffer);

    printf("...");
    print_flanking_dots(
        ref_seq + MIN(match_index - 1, match_index - 1 - flanking_seq_length), matched_seq);
    print_pattern(pattern_buffer, match_buffer);
    print_flanking_dots(
        match_end, MIN(ref_seq + ref_seq_size, match_end + flanking_seq_length));
    printf("...");
    putchar('\n');

    printf("...");
    print_flanking(ref_seq + MIN(match_index - 1, match_index - 1 - flanking_seq_length),
                   matched_seq);
...
int main(int argc, const char* argv[])
{
    ...
    {
        parse_sam_line(line_buffer, (char*)&read_name_buffer, (char*)ref_name_buffer,
                       &match_index, (char*)cigar_buffer, (char*)pattern_buffer);
