...
static void recursive_generator(const char* pattern,
                                char* buffer,
                                char* cigar,
            ...
            for (const char* a = data->alphabet; *a; a++)
            {
                ...
                *cigar = 'D';
                recursive_generator(pattern, buffer + 1, cigar + 1,
                                    max_edit_distance - 1, data, callback, callback_data);
            }
        }
    }
            ...
            if (*a == *pattern)
            {
                ...
                *cigar = 'M';
                ...
                recursive_generator(pattern + 1, buffer + 1, cigar + 1,
                                    max_edit_distance, data, callback, callback_data);
            }
            else
            {
                *buffer = *a;
...
void generate_all_neighbours(const char* pattern,
                             ...
                             void* callback_data)
{
    size_t n = strlen(pattern) + (size_t)max_edit_distance + 1;
    char buffer[n];
