...
static inline int mem_infer_dir(int64_t l_pac, int64_t b1, int64_t b2,
                                int64_t *dist) {
    int64_t p2;
    ...
    p2 = r1 == r2
             ? b2
             : (l_pac << 1) - 1 -
                   b2; // p2 is the coordinate of read 2 on the read 1 strand
...
static int cal_sub(const mem_opt_t *opt, mem_alnreg_v *r) {
    ...
    for (j = 1; j < r->n; ++j) { // choose unique alignment
        ...
        if (e_min > b_max) { // have overlap
            int min_l = r->a[j].qe - r->a[j].qb < r->a[0].qe - r->a[0].qb
                            ? r->a[j].qe - r->a[j].qb
                            : r->a[0].qe - r->a[0].qb;
...
void mem_pestat(const mem_opt_t *opt, int64_t l_pac, int n,
    ...
    if (bwa_verbose >= 3)
        fprintf(stderr,
                "[M::%s] # candidate unique pairs for (FF, FR, RF, RR): (%ld, "
                "%ld, %ld, %ld)\n",
    ...
    for (d = 0; d < 4; ++d) {
        ...
        if (q->n < MIN_DIR_CNT) {
            ...
            fprintf(stderr,
                    "[M::%s] analyzing insert size distribution for "
                    "orientation %c%c...\n",
                    __func__, "FR"[d >> 1 & 1], "FR"[d & 1]);
        ...
        fprintf(stderr,
                "[M::%s] low and high boundaries for computing mean and "
                "std.dev: (%d, %d)\n",
        ...
        if (r->low < 1)
            r->low = 1;
        fprintf(stderr,
                "[M::%s] low and high boundaries for proper pairs: (%d, %d)\n",
                __func__, r->low, r->high);
        free(q->a);
    }
    for (d = 0, max = 0; d < 4; ++d)
...
void mem_pestat(const mem_opt_t *opt, int64_t l_pac, int n, const mem_alnreg_v *regs, mem_pestat_t pes[4])
{
	...
	for (d = 0; d < 4; ++d) { // TODO: this block is nearly identical to the one in bwtsw2_pair.c. It would be better to merge these two. id:13 gh:31 ic:gh
		mem_pestat_t *r = &pes[d];
		uint64_v *q = &isize[d];
		int p25, p50, p75, x;
		if (q->n < MIN_DIR_CNT) {
    ...
    for (r = 0; r < 4; ++r) {
        ...
        if (!is_rev) {
            ...
            re = (is_larger ? a->rb + pes[r].high : a->rb - pes[r].low) +
                 l_ms; // if on the same strand, end position should be larger
                       // to make room for the seq length
        } else {
            rb = (is_larger ? a->rb + pes[r].low : a->rb - pes[r].high) -
                 l_ms; // similarly on opposite strands
            re = is_larger ? a->rb + pes[r].high : a->rb - pes[r].low;
        }
        if (rb < 0)
            rb = 0;
        ...
        if (a->rid == rid &&
            re - rb >= opt->min_seed_len) { // no funny things happening
            kswr_t aln;
            mem_alnreg_t b;
            int tmp, xtra = KSW_XSUBO | KSW_XSTART |
                            (l_ms * opt->a < 250 ? KSW_XBYTE : 0) |
                            (opt->min_seed_len * opt->a);
            aln = ksw_align2(l_ms, seq, re - rb, ref, 5, opt->mat, opt->o_del,
                             opt->e_del, opt->o_ins, opt->e_ins, xtra, 0);
            memset(&b, 0, sizeof(mem_alnreg_t));
            if (aln.score >= opt->min_seed_len &&
                aln.qb >= 0) { // something goes wrong if aln.qb < 0
                b.rid = a->rid;
                b.is_alt = a->is_alt;
                ...
                b.secondary = -1;
                b.seedcov =
                    (b.re - b.rb < b.qe - b.qb ? b.re - b.rb : b.qe - b.qb) >>
                    1;
                //				printf("*** %d, [%lld,%lld], %d:%d, (%lld,%lld),
                //(%lld,%lld) == (%lld,%lld)\n", aln.score, rb, re, is_rev,
                // is_larger, a->rb, a->re, ma->a[0].rb, ma->a[0].re, b.rb,
                // b.re);
...
<<<<<<< HEAD
int mem_pair(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac,
             const mem_pestat_t pes[4], bseq1_t s[2], mem_alnreg_v a[2], int id,
    ...
    for (r = 0; r < 2; ++r) { // loop through read number
        for (i = 0; i < n_pri[r]; ++i) {
            ...
            key.x = e->rb < l_pac
                        ? e->rb
                        : (l_pac << 1) - 1 - e->rb; // forward position
            key.x = (uint64_t)e->rid << 32 | (key.x - bns->anns[e->rid].offset);
            key.y =
                (uint64_t)e->score << 32 | i << 2 | (e->rb >= l_pac) << 1 | r;
    ...
    // (int)(v.a[i].y&1)+1, "+-"[v.a[i].y>>1&1], (long)v.a[i].x);
    for (i = 0; i < v.n; ++i) {
        for (r = 0; r < 2; ++r) { // loop through direction
            ...
            for (k = y[which]; k >= 0; --k) {
                ...
                q = (int)((v.a[i].y >> 32) + (v.a[k].y >> 32) +
                          .721 * log(2. * erfc(fabs(ns) * M_SQRT1_2)) * opt->a +
                          .499); // .721 = 1/log(4)
                if (q < 0)
                    q = 0;
                ...
                p->y = (uint64_t)k << 32 | i;
                p->x =
                    (uint64_t)q << 32 | (hash_64(p->y ^ id << 8) & 0xffffffffU);
...
int mem_pair(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac, const mem_pestat_t pes[4], bseq1_t s[2], mem_alnreg_v a[2], int id, int *sub, int *n_sub, int z[2], int n_pri[2])
{
	...
	for (i = 0; i < v.n; ++i) {
		for (r = 0; r < 2; ++r) { // loop through direction
			...
			for (k = y[which]; k >= 0; --k) { // TODO: this is a O(n^2) solution in the worst case; remember to check if this loop takes a lot of time (I doubt) id:19 gh:37 ic:gh
				int64_t dist;
				int q;
				double ns;
				pair64_t *p;
...
int mem_sam_pe(const mem_opt_t *opt, const bntseq_t *bns, const uint8_t *pac,
               ...
               mem_alnreg_v a[2]) {
    extern int mem_mark_primary_se(const mem_opt_t *opt, int n, mem_alnreg_t *a,
                                   int64_t id);
    extern int mem_approx_mapq_se(const mem_opt_t *opt, const mem_alnreg_t *a);
    extern void mem_reg2sam(const mem_opt_t *opt, const bntseq_t *bns,
                            const uint8_t *pac, bseq1_t *s, mem_alnreg_v *a,
                            int extra_flag, const mem_aln_t *m);
    extern char **mem_gen_alt(const mem_opt_t *opt, const bntseq_t *bns,
                              const uint8_t *pac, const mem_alnreg_v *a,
                              int l_query, const char *query);
...
<<<<<<< HEAD
    ...
    n_aa[0] = n_aa[1] = 0;
    if (!(opt->flag &
          MEM_F_NO_RESCUE)) { // then perform SW for the best alignment
        mem_alnreg_v b[2];
        kv_init(b[0]);
        kv_init(b[1]);
        for (i = 0; i < 2; ++i)
        ...
        for (i = 0; i < 2; ++i)
            for (j = 0; j < b[i].n && j < opt->max_matesw; ++j)
                n += mem_matesw(opt, bns, pac, pes, &b[i].a[j], s[!i].l_seq,
                                (uint8_t *)s[!i].seq, &a[!i]);
        free(b[0].a);
        free(b[1].a);
    }
    ...
    if (n_pri[0] && n_pri[1] &&
        ...
        // q_pe = o && subo < o? (int)(MEM_MAPQ_COEF * (1. - (double)subo / o) *
        // log(a[0].a[z[0]].seedcov + a[1].a[z[1]].seedcov) + .499) : 0;
        ...
        if (q_pe > 60)
            q_pe = 60;
        q_pe =
            (int)(q_pe * (1. - .5 * (a[0].a[0].frac_rep + a[1].a[0].frac_rep)) +
                  .499);
        ...
        if (o > score_un) { // paired alignment is preferred
            ...
            for (i = 0; i < 2; ++i) {
                if (c[i]->secondary >= 0)
                    c[i]->sub = a[i].a[c[i]->secondary].score,
                    c[i]->secondary = -2;
                q_se[i] = mem_approx_mapq_se(opt, c[i]);
            }
            q_se[0] = q_se[0] > q_pe
                          ? q_se[0]
                          : q_pe < q_se[0] + 40 ? q_pe : q_se[0] + 40;
            q_se[1] = q_se[1] > q_pe
                          ? q_se[1]
                          : q_pe < q_se[1] + 40 ? q_pe : q_se[1] + 40;
            extra_flag |= 2;
            // cap at the tandem repeat score
            q_se[0] = q_se[0] < raw_mapq(c[0]->score - c[0]->csub, opt->a)
                          ? q_se[0]
                          : raw_mapq(c[0]->score - c[0]->csub, opt->a);
            q_se[1] = q_se[1] < raw_mapq(c[1]->score - c[1]->csub, opt->a)
                          ? q_se[1]
        ...
        for (i = 0; i < 2; ++i) {
            ...
            if (k >= 0 && k < n_pri[i]) { // switch secondary and primary if
                                          // both of them are non-ALT
        ...
        for (i = 0; i < 2; ++i) {
            h[i] =
                mem_reg2aln(opt, bns, pac, s[i].l_seq, s[i].seq, &a[i].a[z[i]]);
            h[i].mapq = q_se[i];
        ...
        for (i = 0; i < n_aa[0]; ++i)
            mem_aln2sam(opt, bns, &str, &s[0], n_aa[0], aa[0], i,
                        &h[1]); // write read1 hits
        s[0].sam = strdup(str.s);
        str.l = 0;
        for (i = 0; i < n_aa[1]; ++i)
            mem_aln2sam(opt, bns, &str, &s[1], n_aa[1], aa[1], i,
                        &h[0]); // write read2 hits
        s[1].sam = str.s;
        if (strcmp(s[0].name, s[1].name) != 0)
            err_fatal(__func__,
                      "paired reads have different names: \"%s\", \"%s\"\n",
                      s[0].name, s[1].name);
        // free
        for (i = 0; i < 2; ++i) {
            free(h[i].cigar);
...
=======
	...
	if (n_pri[0] && n_pri[1] && (o = mem_pair(opt, bns, pac, pes, s, a, id, &subo, &n_sub, z, n_pri)) > 0) {
		...
		if (is_multi[0] || is_multi[1]) goto no_pairing; // TODO: in rare cases, the true hit may be long but with low score id:25 gh:44 ic:gh
		// compute mapQ for the best SE hit
		score_un = a[0].a[0].score + a[1].a[0].score - opt->pen_unpaired;
...
no_pairing:
    for (i = 0; i < 2; ++i) {
        ...
        if (which >= 0)
            h[i] = mem_reg2aln(opt, bns, pac, s[i].l_seq, s[i].seq,
                               &a[i].a[which]);
        else
            h[i] = mem_reg2aln(opt, bns, pac, s[i].l_seq, s[i].seq, 0);
    }
    if (!(opt->flag & MEM_F_NOPAIRING) && h[0].rid == h[1].rid &&
        h[0].rid >= 0) { // if the top hits from the two ends constitute a
                         // proper pair, flag it.
        int64_t dist;
        int d;
