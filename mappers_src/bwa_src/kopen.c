...
static int http_open(const char *fn) {
    ...
    if (proxy == 0) {
        host = strdup(http_host); // when there is no proxy, server name is
                                  // identical to http_host name.
        port = strdup(*q ? q : "80");
        path = strdup(*p ? p : "/");
    } else {
        host = (strstr(proxy, "http://") == proxy) ? strdup(proxy + 7)
                                                   : strdup(proxy);
        for (q = host; *q && *q != ':'; ++q)
            ;
        if (*q == ':')
...
retry:
    while (l < bufsz &&
           (bytes = read(fd, buf + l, 1)) > 0) { // read HTTP header;
        if (buf[l] == '\n' && l >= 3)
            if (strncmp(buf + l - 3, "\r\n\r\n", 4) == 0)
                break;
        ++l;
...
static int kftp_get_response(ftpaux_t *aux) {
    ...
    while (read(aux->ctrl_fd, &c,
        ...
        aux->response[n++] = c;
        if (c == '\n') {
            if (n >= 4 && isdigit(aux->response[0]) &&
                isdigit(aux->response[1]) && isdigit(aux->response[2]) &&
                aux->response[3] != '-')
                break;
            n = 0;
            continue;
...
static int ftp_open(const char *fn) {
    ...
    sprintf(host2, "%d.%d.%d.%d", pasv_ip[0], pasv_ip[1], pasv_ip[2],
            pasv_ip[3]);
    ...
    *_fd = -1;
    if (strstr(fn, "http://") == fn) {
        ...
        if (*p == '<') { // pipe open
            ...
            pid_t pid;
            // a simple check to see if we need to invoke a shell; not always
            // working
            for (q = p + 1; *q; ++q)
                if (ispunct(*q) && *q != '.' && *q != '_' && *q != '-' &&
                    *q != ':')
                    break;
            need_shell = (*q != 0);
            if (pipe(pfd) != 0)
