...
void bwa_aln2seq_core(int n_aln, const bwt_aln1_t *aln, bwa_seq_t *s,
    ...
    if (n_multi) {
        ...
        if (s->multi)
            free(s->multi);
        if (n_occ >
            n_multi + 1) { // if there are too many hits, generate none of them
            s->multi = 0;
            s->n_multi = 0;
            return;
        }
        ...
        rest = n_occ > n_multi + 1 ? n_multi + 1
                                   : n_occ; // find one additional for ->sa
        ...
        for (k = 0; k < n_aln; ++k) {
            ...
            if (q->l - q->k + 1 <= rest) {
                ...
                rest -= q->l - q->k + 1;
                     ...
                     // (http://code.activestate.com/recipes/272884/). In fact,
                     // we never come here.
                int j, i;
...
bwtint_t bwa_sa2pos(const bntseq_t *bns, const bwt_t *bwt, bwtint_t sapos,
    ...
    bwtint_t pos_f;
    int is_rev;
    *strand = 0; // initialise strand to 0 otherwise we could return without
                 // setting it
    ...
    pos_f = bns_depos(bns, pos_f,
                      &is_rev); // position on the forward strand; this may be
                                // the first base or the last base
    *strand = !is_rev;
    if (is_rev)
        pos_f = pos_f + 1 < ref_len
                    ? 0
 ...
 */
void bwa_cal_pac_pos_core(const bntseq_t *bns, const bwt_t *bwt, bwa_seq_t *seq,
                          const int max_mm, const float fnr) {
    int max_diff, strand;
    ...
    // fprintf(stderr, "%d\n", seq->ref_shift);
    seq->pos =
        bwa_sa2pos(bns, bwt, seq->sa, seq->len + seq->ref_shift, &strand);
    seq->strand = strand;
    seq->seQ = seq->mapQ = bwa_approx_mapQ(seq, max_diff);
...
void bwa_cal_pac_pos(const bntseq_t *bns, const char *prefix, int n_seqs,
    ...
    for (i = 0; i != n_seqs; ++i) {
        ...
        for (j = n_multi = 0; j < p->n_multi; ++j) {
            ...
            q->pos =
                bwa_sa2pos(bns, bwt, q->pos, p->len + q->ref_shift, &strand);
            q->strand = strand;
...
bwa_cigar_t *bwa_refine_gapped_core(bwtint_t l_pac, const ubyte_t *pacseq,
                                    int len, ubyte_t *seq, int ref_shift,
                                    bwtint_t *_rb, int *n_cigar) {
    bwa_cigar_t *cigar = 0;
    uint32_t *cigar32 = 0;
    ubyte_t *rseq;
    ...
    ksw_global(len, seq, rlen, rseq, 5, mat, 5, 1, SW_BW > w ? SW_BW : w,
               n_cigar, &cigar32);
    assert(*n_cigar > 0);
    if ((cigar32[*n_cigar - 1] & 0xf) == 1)
        cigar32[*n_cigar - 1] = (cigar32[*n_cigar - 1] >> 4 << 4) |
                                3; // change endding ins to soft clipping
    if ((cigar32[0] & 0xf) == 1)
        cigar32[0] =
            (cigar32[0] >> 4 << 4) | 3; // change beginning ins to soft clipping
...
char *bwa_cal_md1(int n_cigar, bwa_cigar_t *cigar, int len, bwtint_t pos,
                  ubyte_t *seq, bwtint_t l_pac, ubyte_t *pacseq, kstring_t *str,
                  int *_nm) {
    bwtint_t x, y;
    int z, u, c, nm = 0;
    str->l = 0; // reset
    ...
    if (cigar) {
        ...
        for (k = u = 0; k < n_cigar; ++k) {
            ...
            if (__cigar_op(cigar[k]) == FROM_M) {
                ...
                kputc('^', str);
                for (z = 0; z < l && x + z < l_pac; ++z)
                    kputc("ACGT"[pacseq[(x + z) >> 2] >> ((~(x + z) & 3) << 1) &
                                 3],
                          str);
                u = 0;
                x += l;
...
void bwa_correct_trimmed(bwa_seq_t *s) {
    ...
    if (s->strand == 0) { // forward
        if (s->cigar &&
            __cigar_op(s->cigar[s->n_cigar - 1]) == FROM_S) { // the last is S
            ...
            s->cigar[s->n_cigar - 1] =
                __cigar_create(3, (s->full_len - s->len));
        }
    } else {                                                 // reverse
        if (s->cigar && __cigar_op(s->cigar[0]) == FROM_S) { // the first is S
            ...
            if (s->cigar == 0) {
                ...
                ++s->n_cigar;
                s->cigar = realloc(s->cigar, s->n_cigar * sizeof(bwa_cigar_t));
                memmove(s->cigar + 1, s->cigar,
                        (s->n_cigar - 1) * sizeof(bwa_cigar_t));
            }
...
void bwa_refine_gapped(const bntseq_t *bns, int n_seqs, bwa_seq_t *seqs,
                       ubyte_t *_pacseq) {
    ubyte_t *pacseq;
    int i, j, k;
    kstring_t *str;
    ...
    for (i = 0; i != n_seqs; ++i) {
        ...
        for (j = k = 0; j < s->n_multi; ++j) {
            ...
            if (q->gap) { // gapped alignment
                q->cigar = bwa_refine_gapped_core(
                    bns->l_pac, pacseq, s->len, q->strand ? s->rseq : s->seq,
        ...
        s->n_multi = k; // this squeezes out gapped alignments which failed the
                        // CIGAR generation
        if (s->type == BWA_TYPE_NO_MATCH || s->type == BWA_TYPE_MATESW ||
            s->n_gapo == 0)
            continue;
        s->cigar = bwa_refine_gapped_core(bns->l_pac, pacseq, s->len,
                                          s->strand ? s->rseq : s->seq,
                                          s->ref_shift, &s->pos, &s->n_cigar);
        if (s->cigar == 0)
            s->type = BWA_TYPE_NO_MATCH;
    }
    // generate MD tag
    ...
    for (i = 0; i != n_seqs; ++i) {
        ...
        if (s->type != BWA_TYPE_NO_MATCH) {
            int nm;
            s->md = bwa_cal_md1(s->n_cigar, s->cigar, s->len, s->pos,
                                s->strand ? s->rseq : s->seq, bns->l_pac,
                                pacseq, str, &nm);
            s->nm = nm;
        }
    }
...
void bwa_print_sam1(const bntseq_t *bns, bwa_seq_t *p, const bwa_seq_t *mate,
                    int mode, int max_top2) {
    int j;
    if (p->type != BWA_TYPE_NO_MATCH ||
        ...
        if (p->type != BWA_TYPE_NO_MATCH &&
            ...
            flag |= SAM_FSU; // flag UNMAP as this alignment bridges two
                             // adjacent reference sequences
        ...
        err_printf("%d\t%d\t", (int)(p->pos - bns->anns[seqid].offset + 1),
                   p->mapQ);
        ...
        if (mate && mate->type != BWA_TYPE_NO_MATCH) {
            ...
            err_printf("\t%s\t",
                       (seqid == m_seqid) ? "=" : bns->anns[m_seqid].name);
            isize = (seqid == m_seqid) ? pos_5(mate) - pos_5(p) : 0;
            if (p->type == BWA_TYPE_NO_MATCH)
                isize = 0;
            err_printf("%d\t%lld\t",
                       (int)(mate->pos - bns->anns[m_seqid].offset + 1), isize);
        } else if (mate)
            err_printf("\t=\t%d\t0\t",
                       (int)(p->pos - bns->anns[seqid].offset + 1));
        else
            err_printf("\t*\t0\t0\t");
        ...
        if (p->type != BWA_TYPE_NO_MATCH) {
            ...
            err_printf("\tXT:A:%c\t%s:i:%d", XT,
                       (mode & BWA_MODE_COMPREAD) ? "NM" : "CM", p->nm);
            if (nn)
                err_printf("\tXN:i:%d", nn);
            if (mate)
                err_printf("\tSM:i:%d\tAM:i:%d", p->seQ, am);
            if (p->type != BWA_TYPE_MATESW) { // X0 and X1 are not available for
                                              // this type of alignment
                err_printf("\tX0:i:%d", p->c1);
                if (p->c1 <= max_top2)
                    err_printf("\tX1:i:%d", p->c2);
            }
            err_printf("\tXM:i:%d\tXO:i:%d\tXG:i:%d", p->n_mm, p->n_gapo,
                       p->n_gapo + p->n_gape);
            if (p->md)
            ...
            if (p->n_multi) {
                ...
                for (i = 0; i < p->n_multi; ++i) {
                    ...
                    err_printf("%s,%c%d,", bns->anns[seqid].name,
                               q->strand ? '-' : '+',
...
void bwa_sai2sam_se_core(const char *prefix, const char *fn_sa,
    ...
    if (strncmp(magic, SAI_MAGIC, 4) != 0) {
        fprintf(stderr,
                "[E::%s] Unmatched SAI magic. Please re-run `aln' with the "
                "same version of bwa.\n",
                __func__);
        exit(1);
    }
    ...
    // core loop
    while ((seqs = bwa_read_seq(ks, 0x40000, &n_seqs, opt.mode,
                                opt.trim_qual)) != 0) {
        tot_seqs += n_seqs;
        t = clock();
        ...
        bwa_cal_pac_pos(bns, prefix, n_seqs, seqs, opt.max_diff,
...
int bwa_sai2sam_se(int argc, char *argv[]) {
    ...
    if (optind + 3 > argc) {
        fprintf(stderr, "Usage: bwa samse [-n max_occ] [-f out.sam] [-r "
                        "RG_line] <prefix> <in.sai> <in.fq>\n");
        return 1;
    }
