...
bsw2pestat_t bsw2_stat(int n, bwtsw2_t **buf, kstring_t *msg, int max_ins) {
    ...
    for (i = k = 0; i < n; i += 2) {
        ...
        if (t[1]->G2 > 0.8 * t[1]->G)
            continue; // the best hit is not good enough
        l = t[0]->k > t[1]->k ? t[0]->k - t[1]->k + t[1]->len
                              : t[1]->k - t[0]->k + t[0]->len;
        if (l >= max_ins)
            continue; // skip pairs with excessively large insert
        max_len =
            max_len > t[0]->end - t[0]->beg ? max_len : t[0]->end - t[0]->beg;
        max_len =
            max_len > t[1]->end - t[1]->beg ? max_len : t[1]->end - t[1]->beg;
        isize[k++] = l;
    }
    ...
    ksprintf(
        msg,
        "[%s] infer the insert size distribution from %d high-quality pairs.\n",
        __func__, k);
    if (k < 8) {
        ksprintf(msg,
                 "[%s] fail to infer the insert size distribution: too few "
                 "good pairs.\n",
                 __func__);
        free(isize);
        r.failed = 1;
    ...
    if (r.low > r.high) {
        ksprintf(msg,
                 "[%s] fail to infer the insert size distribution: upper bound "
                 "is smaller than max read length.\n",
                 __func__);
        free(isize);
        r.failed = 1;
        return r;
    ...
    ksprintf(msg, "[%s] (25, 50, 75) percentile: (%d, %d, %d)\n", __func__, p25,
             p50, p75);
    ksprintf(msg,
             "[%s] low and high boundaries for computing mean and std.dev: "
             "(%d, %d)\n",
    ...
    if (x == 0) {
        ksprintf(msg,
                 "[%s] fail to infer the insert size distribution: no pairs "
                 "within boundaries.\n",
                 __func__);
        free(isize);
        r.failed = 1;
...
void bsw2_pair1(const bsw2opt_t *opt, int64_t l_pac, const uint8_t *pac,
                const bsw2pestat_t *st, const bsw2hit_t *h, int l_mseq,
                const char *mseq, bsw2hit_t *a, int8_t g_mat[25]) {
    ...
    a->n_seeds = 1;
    a->flag |=
        BSW2_FLAG_MATESW; // before calling this routine, *a has been cleared
                          // with memset(0); the flag is set with 1<<6/7
    if (h->is_rev == 0) {
        ...
        beg = (int64_t)(h->k + h->end - h->beg - st->avg -
                        EXT_STDDEV * st->std + .499);
        end = (int64_t)(h->k + h->end - h->beg - st->avg +
                        EXT_STDDEV * st->std + l_mseq + .499);
    ...
    {
        int flag = KSW_XSUBO | KSW_XSTART |
                   (l_mseq * g_mat[0] < 250 ? KSW_XBYTE : 0) | opt->t;
        kswr_t aln;
        aln = ksw_align(l_mseq, seq, end - beg, ref, 5, g_mat, opt->q, opt->r,
                        flag, 0);
        a->G = aln.score;
        a->G2 = aln.score2;
        if (a->G < opt->t)
        ...
        a->end = aln.qe + 1;
        /*
        printf("[Q] "); for (i = 0; i < l_mseq; ++i)
        putchar("ACGTN"[(int)seq[i]]); putchar('\n'); printf("[R] "); for (i =
        0; i < end - beg; ++i) putchar("ACGTN"[(int)ref[i]]); putchar('\n');
...
void bsw2_pair(const bsw2opt_t *opt, int64_t l_pac, const uint8_t *pac, int n,
    ...
    for (i = 0; i < n; i += 2) {
        ...
        if (!opt->skip_sw) {
            if (hits[i + 0]->n == 1)
                bsw2_pair1(opt, l_pac, pac, &pes, &hits[i + 0]->hits[0],
                           seq[i + 1].l, seq[i + 1].seq, &a[1], g_mat);
            if (hits[i + 1]->n == 1)
                bsw2_pair1(opt, l_pac, pac, &pes, &hits[i + 1]->hits[0],
                           seq[i + 0].l, seq[i + 0].seq, &a[0], g_mat);
        } // else a[0].G == a[1].G == a[0].G2 == a[1].G2 == 0
        // the following enumerate all possibilities. It is tedious but
        // necessary...
        if (hits[i]->n + hits[i + 1]->n ==
            1) { // one end mapped; the other not;
            bwtsw2_t *p[2];
            int which;
            if (hits[i]->n == 1)
            ...
            // hits[i]->hits[0].k, a[0].k, hits[i]->hits[0].end, a[0].end);
            for (j = 0; j < 2; ++j) { // fix wrong mappings and wrong suboptimal
                                      // alignment score
                bsw2hit_t *p = &hits[i + j]->hits[0];
                if (p->G < a[j].G) { // the orginal mapping is suboptimal
                    a[j].G2 = a[j].G2 > p->G ? a[j].G2 : p->G;
                    *p = a[j];
                    ++n_fixed;
                    is_fixed = 1;
            ...
            if (hits[i]->hits[0].k == a[0].k &&
                hits[i + 1]->hits[0].k ==
                    a[1].k) { // properly paired and no ends need to be moved
                for (j = 0; j < 2; ++j)
                    hits[i + j]->hits[0].flag |=
                        2 | (a[j].flag & BSW2_FLAG_TANDEM);
            } else if (hits[i]->hits[0].k == a[0].k ||
                       hits[i + 1]->hits[0].k == a[1].k) { // a tandem match
                for (j = 0; j < 2; ++j) {
                    ...
                    if (hits[i + j]->hits[0].k != a[j].k)
                       ...
                       (a[0].G || a[1].G)) { // it is possible to move one end
                if (a[0].G && a[1].G) {      // now we have two "proper pairs"
                    ...
                    diff = fabs((double)(G[0] - G[1])) / (opt->a + opt->b) /
                           ((hits[i]->hits[0].len + a[1].len +
                             hits[i + 1]->hits[0].len + a[0].len) /
                            2.);
                    if (diff > 0.05)
                        a[G[0] > G[1] ? 0 : 1].G = 0;
                }
                if (a[0].G == 0 || a[1].G == 0) { // one proper pair only
                    bsw2hit_t *p[2]; // p[0] points the unchanged hit; p[1] to
                                     // the hit to be moved
                    int which, isize;
                    double dev, diff;
                    if (a[0].G)
                        p[0] = &hits[i + 1]->hits[0], p[1] = &hits[i]->hits[0],
                        which = 0;
                    else
                        p[0] = &hits[i]->hits[0], p[1] = &hits[i + 1]->hits[0],
                        which = 1;
                    isize = p[0]->is_rev ? p[0]->k + p[0]->len - a[which].k
                                         : a[which].k + a[which].len - p[0]->k;
                    dev = fabs(isize - pes.avg) / pes.std;
                    diff = (double)(p[1]->G - a[which].G) / (opt->a + opt->b) /
                           (p[1]->end - p[1]->beg) * 100.0;
