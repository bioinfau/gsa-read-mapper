...
int bwa_pemerge(const pem_opt_t *opt, bseq1_t x[2]) {
    ...
    r = ksw_align(x[1].l_seq, s[1], x[0].l_seq, s[0], 5, opt->mat, opt->q,
                  opt->r, xtra, 0);
    ++r.qe;
...
<<<<<<< HEAD
    { // test tandem match; O(n^2)
        ...
        for (l = 1; l < min_l; ++l) {
            ...
            for (i = 0; i < l; ++i)
                m += opt->mat[(s1[i] << 2) + s1[i] +
                              s0o[i]]; // equivalent to s[1][i]*5 + s[0][o+i]
            if (m > max_m)
        ...
        if (max_l2 < max_l && max_m2 >= opt->T &&
            (double)(max_m2 + (max_l - max_l2) * opt->a) / max_m >=
                MAX_SCORE_RATIO) {
            ret = -7;
            goto pem_ret;
        }
...
static void process_seqs(const pem_opt_t *opt, int n_, bseq1_t *seqs,
                         int64_t cnt[MAX_ERR + 1]) {
    int i, j, n = n_ >> 1 << 1;
    worker_t *w;
...
int main_pemerge(int argc, char *argv[]) {
    ...
    fp = strcmp(argv[optind], "-") ? gzopen(argv[optind], "r")
    ...
    ks = kseq_init(fp);
    if (optind + 1 < argc) {
        fp2 = strcmp(argv[optind + 1], "-") ? gzopen(argv[optind + 1], "r")
                                            : gzdopen(fileno(stdin), "r");
        if (NULL == fp) {
            fprintf(stderr, "Couldn't open %s : %s\n",
