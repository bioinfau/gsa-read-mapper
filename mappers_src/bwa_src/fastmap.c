...
static void *process(void *shared, int step, void *_data) {
    ...
    if (step == 0) {
        ...
        ret->seqs =
            bseq_read(aux->actual_chunk_size, &ret->n_seqs, aux->ks, aux->ks2);
        if (ret->seqs == 0) {
            free(ret);
            return 0;
        }
        ...
        if (opt->flag & MEM_F_SMARTPE) {
            ...
            if (bwa_verbose >= 3)
                fprintf(stderr,
                        "[M::%s] %d single-end sequences; %d paired-end "
                        "sequences\n",
            ...
            if (n_sep[1]) {
                tmp_opt.flag |= MEM_F_PE;
                mem_process_seqs(&tmp_opt, idx->bwt, idx->bns, idx->pac,
                                 aux->n_processed + n_sep[0], n_sep[1], sep[1],
                                 aux->pes0);
                for (i = 0; i < n_sep[1]; ++i)
            ...
            mem_process_seqs(opt, idx->bwt, idx->bns, idx->pac,
                             aux->n_processed, data->n_seqs, data->seqs,
                             aux->pes0);
...
<<<<<<< HEAD
    ...
    while ((c = getopt(argc, argv,
                       "51qpaMCSPVYjk:c:v:s:r:t:R:A:B:O:E:U:w:L:d:T:Q:D:m:I:N:"
                       "o:f:W:x:G:h:y:K:X:H:")) >= 0) {
        if (c == 'k')
            ...
            opt->n_threads = opt->n_threads > 1 ? opt->n_threads : 1;
        else if (c == 'P')
            ...
            opt->flag |= MEM_F_PRIMARY5 |
                         MEM_F_KEEP_SUPP_MAPQ; // always apply
                                               // MEM_F_KEEP_SUPP_MAPQ with -5
        else if (c == 'q')
            ...
            opt->mapQ_coef_fac =
                opt->mapQ_coef_len > 0 ? log(opt->mapQ_coef_len) : 0;
        } else if (c == 'O') {
            ...
            if (bwa_verbose >= 3)
                fprintf(stderr,
                        "[M::%s] mean insert size: %.3f, stddev: %.3f, max: "
                        "%d, min: %d\n",
                        __func__, pes[1].avg, pes[1].std, pes[1].high,
                        pes[1].low);
        } else
            return 1;
    }
=======
	...
	while ((c = getopt(argc, argv, "51qpaMCSPVYjk:c:v:s:r:t:R:A:B:O:E:U:w:L:d:T:Q:D:m:I:N:o:f:W:x:G:h:y:K:X:H:")) >= 0) {
		if (c == 'k') opt->min_seed_len = atoi(optarg), opt0.min_seed_len = 1;
			...
			if ((rg_line = bwa_set_rg(optarg)) == 0) return 1; // FIXME: memory leak id:21 gh:39 ic:gh
		} else if (c == 'H') {
			if (optarg[0] != '@') {
				FILE *fp;
				if ((fp = fopen(optarg, "r")) != 0) {
...
>>>>>>> d8543aff45a0e8a3fdc7c7977c8f9021966aad1f
    ...
    if (optind + 1 >= argc || optind + 3 < argc) {
        ...
        fprintf(stderr, "       -k INT        minimum seed length [%d]\n",
                opt->min_seed_len);
        fprintf(stderr,
                "       -w INT        band width for banded alignment [%d]\n",
                opt->w);
        fprintf(stderr, "       -d INT        off-diagonal X-dropoff [%d]\n",
                opt->zdrop);
        fprintf(stderr,
                "       -r FLOAT      look for internal seeds inside a seed "
                "longer than {-k} * FLOAT [%g]\n",
                opt->split_factor);
        fprintf(stderr,
                "       -y INT        seed occurrence for the 3rd round "
                "seeding [%ld]\n",
                (long)opt->max_mem_intv);
        //		fprintf(stderr, "       -s INT        look for internal seeds
        // inside a seed with less than INT occ [%d]\n", opt->split_width);
        fprintf(stderr,
                "       -c INT        skip seeds with more than INT "
                "occurrences [%d]\n",
                opt->max_occ);
        fprintf(stderr,
                "       -D FLOAT      drop chains shorter than FLOAT fraction "
                "of the longest overlapping chain [%.2f]\n",
                opt->drop_ratio);
        fprintf(stderr, "       -W INT        discard a chain if seeded bases "
                        "shorter than INT [0]\n");
        fprintf(stderr,
                "       -m INT        perform at most INT rounds of mate "
                "rescues for each read [%d]\n",
                opt->max_matesw);
        fprintf(stderr, "       -S            skip mate rescue\n");
        fprintf(stderr, "       -P            skip pairing; mate rescue "
                        "performed unless -S also in use\n");
        fprintf(stderr, "\nScoring options:\n\n");
        fprintf(stderr,
                "       -A INT        score for a sequence match, which scales "
                "options -TdBOELU unless overridden [%d]\n",
                opt->a);
        ...
        fprintf(
            stderr,
            "       -U INT        penalty for an unpaired read pair [%d]\n\n",
            opt->pen_unpaired);
        fprintf(stderr, "       -x STR        read type. Setting -x changes "
                        "multiple parameters unless overridden [null]\n");
        fprintf(stderr, "                     pacbio: -k17 -W40 -r10 -A1 -B1 "
                        "-O1 -E1 -L0  (PacBio reads to ref)\n");
        fprintf(stderr, "                     ont2d: -k14 -W20 -r10 -A1 -B1 "
                        "-O1 -E1 -L0  (Oxford Nanopore 2D-reads to ref)\n");
        fprintf(stderr, "                     intractg: -B9 -O16 -L5  "
                        "(intra-species contigs to ref)\n");
        fprintf(stderr, "\nInput/output options:\n\n");
        fprintf(stderr,
                "       -p            smart pairing (ignoring in2.fq)\n");
        fprintf(stderr, "       -R STR        read group header line such as "
                        "'@RG\\tID:foo\\tSM:bar' [null]\n");
        fprintf(stderr, "       -H STR/FILE   insert STR to header if it "
                        "starts with @; or insert lines in FILE [null]\n");
        fprintf(
            stderr,
            "       -o FILE       sam file to output results to [stdout]\n");
        fprintf(stderr, "       -j            treat ALT contigs as part of the "
                        "primary assembly (i.e. ignore <idxbase>.alt file)\n");
        fprintf(stderr, "       -5            for split alignment, take the "
                        "alignment with the smallest coordinate as primary\n");
        fprintf(stderr, "       -q            don't modify mapQ of "
                        "supplementary alignments\n");
        fprintf(stderr,
                "       -K INT        process INT input bases in each batch "
                "regardless of nThreads (for reproducibility) []\n");
        fprintf(stderr, "\n");
        fprintf(stderr,
        ...
        fprintf(stderr, "       -T INT        minimum score to output [%d]\n",
                opt->T);
        fprintf(stderr,
                "       -h INT[,INT]  if there are <INT hits with score >80%% "
                "of the max score, output all in XA [%d,%d]\n",
                opt->max_XA_hits, opt->max_XA_hits_alt);
        fprintf(stderr, "       -a            output all alignments for SE or "
                        "unpaired PE\n");
        fprintf(
            stderr,
            "       -C            append FASTA/FASTQ comment to SAM output\n");
        fprintf(stderr, "       -V            output the reference FASTA "
                        "header in the XR tag\n");
        fprintf(stderr, "       -Y            use soft clipping for "
                        "supplementary alignments\n");
        fprintf(
            stderr,
            "       -M            mark shorter split hits as secondary\n\n");
        fprintf(stderr, "       -I FLOAT[,FLOAT[,INT[,INT]]]\n");
        fprintf(stderr, "                     specify the mean, standard "
                        "deviation (10%% of the mean if absent), max\n");
        fprintf(stderr, "                     (4 sigma from the mean if "
                        "absent) and min of the insert size distribution.\n");
        fprintf(stderr,
                "                     FR orientation only. [inferred]\n");
        fprintf(stderr, "\n");
        fprintf(stderr, "Note: Please read the man page for detailed "
...
<<<<<<< HEAD
    if (mode) {
        if (strcmp(mode, "intractg") == 0) {
            ...
            if (!opt0.pen_clip3)
                opt->pen_clip3 = 5;
        } else if (strcmp(mode, "pacbio") == 0 || strcmp(mode, "pbref") == 0 ||
                   strcmp(mode, "ont2d") == 0) {
            if (!opt0.o_del)
                opt->o_del = 1;
            if (!opt0.e_del)
...
=======
	if (mode) {
		if (strcmp(mode, "intractg") == 0) {
			...
			fprintf(stderr, "[E::%s] unknown read type '%s'\n", __func__, mode);
			return 1; // FIXME memory leak id:27 gh:46 ic:gh
		}
	...
	aux.idx = bwa_idx_load_from_shm(argv[optind]);
	if (aux.idx == 0) {
		if ((aux.idx = bwa_idx_load(argv[optind], BWA_IDX_ALL)) == 0) return 1; // FIXME: memory leak id:4 gh:22 ic:gh
	} else if (bwa_verbose >= 3)
		fprintf(stderr, "[M::%s] load the bwa index from shared memory\n", __func__);
	if (ignore_alt)
		for (i = 0; i < aux.idx->bns->n_seqs; ++i)
...
>>>>>>> d8543aff45a0e8a3fdc7c7977c8f9021966aad1f
    ...
    if (optind + 2 < argc) {
        if (opt->flag & MEM_F_PE) {
            if (bwa_verbose >= 2)
                fprintf(stderr,
                        "[W::%s] when '-p' is in use, the second query file is "
                        "ignored.\n",
                        __func__);
        } else {
...
int main_fastmap(int argc, char *argv[]) {
    ...
    if (optind + 1 >= argc) {
        ...
        fprintf(
            stderr,
            "         -w INT    max interval size to find coordiantes [%d]\n",
            min_iwidth);
        fprintf(stderr, "         -i INT    min SMEM interval size [%d]\n",
                min_intv);
        fprintf(stderr, "         -L INT    max MEM length [%d]\n", max_len);
        fprintf(stderr,
                "         -I INT    stop if MEM is longer than -l with a size "
                "less than INT [%ld]\n",
                (long)max_intv);
        fprintf(stderr, "\n");
        return 1;
        ...
        while ((a = smem_next(itr)) != 0) {
            for (i = 0; i < a->n; ++i) {
                ...
                if ((uint32_t)p->info - (p->info >> 32) < min_len)
                    continue;
                err_printf("EM\t%d\t%d\t%ld", (uint32_t)(p->info >> 32),
                           (uint32_t)p->info, (long)p->x[2]);
                if (p->x[2] <= min_iwidth) {
                    for (k = 0; k < p->x[2]; ++k) {
                        bwtint_t pos;
                        int len, is_rev, ref_id;
                        len = (uint32_t)p->info - (p->info >> 32);
                        pos = bns_depos(idx->bns, bwt_sa(idx->bwt, p->x[0] + k),
                                        &is_rev);
                        if (is_rev)
                            pos -= len - 1;
                        bns_cnt_ambi(idx->bns, pos, len, &ref_id);
                        err_printf("\t%s:%c%ld", idx->bns->anns[ref_id].name,
                                   "+-"[is_rev],
                                   (long)(pos - idx->bns->anns[ref_id].offset) +
                                       1);
                    }
                } else
                    err_puts("\t*");
