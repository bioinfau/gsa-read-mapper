...
uint32_t *bwa_gen_cigar2(const int8_t mat[25], int o_del, int e_del, int o_ins,
                         int e_ins, int w_, int64_t l_pac, const uint8_t *pac,
                         int l_query, uint8_t *query, int64_t rb, int64_t re,
    ...
    if (l_query <= 0 || rb >= re || (rb < l_pac && re > l_pac))
        return 0; // reject if negative length or bridging the forward and
                  // reverse strand
    rseq = bns_get_seq(l_pac, pac, rb, re, &rlen);
    if (re - rb != rlen)
        goto ret_gen_cigar; // possible if out of range
    if (rb >= l_pac) { // then reverse both query and rseq; this is to ensure
                       // indels to be placed at the leftmost position
        for (i = 0; i<l_query>> 1; ++i)
            tmp = query[i], query[i] = query[l_query - 1 - i],
            query[l_query - 1 - i] = tmp;
        for (i = 0; i<rlen>> 1; ++i)
            tmp = rseq[i], rseq[i] = rseq[rlen - 1 - i],
            rseq[rlen - 1 - i] = tmp;
    }
    if (l_query == re - rb && w_ == 0) { // no gap; no need to do DP
        // UPDATE: we come to this block now...
        if (n_cigar) {
            cigar = malloc(4);
            cigar[0] = l_query << 4 | 0;
            *n_cigar = 1;
        ...
        // set the band-width
        max_ins =
            (int)((double)(((l_query + 1) >> 1) * mat[0] - o_ins) / e_ins + 1.);
        max_del =
            (int)((double)(((l_query + 1) >> 1) * mat[0] - o_del) / e_del + 1.);
        ...
        *score = ksw_global2(l_query, query, rlen, rseq, 5, mat, o_del, e_del,
    ...
    if (NM && n_cigar) { // compute NM and MD
        ...
        for (k = 0, x = y = u = 0; k < *n_cigar; ++k) {
            ...
            if (op == 0) { // match
                ...
                if (k > 0 &&
                    k < *n_cigar - 1) { // don't do the following if D is the
                                        // first or the last CIGAR
                    kputw(u, &str);
                    kputc('^', &str);
                    for (i = 0; i < len; ++i)
                        kputc(int2base[rseq[y + i]], &str);
...
uint32_t *bwa_gen_cigar(const int8_t mat[25], int q, int r, int w_,
                        ...
                        uint8_t *query, int64_t rb, int64_t re, int *score,
                        int *n_cigar, int *NM) {
    return bwa_gen_cigar2(mat, q, r, q, r, w_, l_pac, pac, l_query, query, rb,
                          re, score, n_cigar, NM);
}
...
int bwa_idx2mem(bwaidx_t *idx) {
    ...
    // copy idx->bns
    tmp = idx->bns->n_seqs * sizeof(bntann1_t) +
          idx->bns->n_holes * sizeof(bntamb1_t);
    for (i = 0; i < idx->bns->n_seqs;
         ++i) // compute the size of heap-allocated memory
        tmp +=
            strlen(idx->bns->anns[i].name) + strlen(idx->bns->anns[i].anno) + 2;
...
void bwa_print_sam_hdr(const bntseq_t *bns, const char *hdr_line) {
    ...
    if (n_SQ == 0) {
        for (i = 0; i < bns->n_seqs; ++i) {
            err_printf("@SQ\tSN:%s\tLN:%d", bns->anns[i].name,
                       bns->anns[i].len);
        ...
        fprintf(stderr,
                "[W::%s] %d @SQ lines provided with -H; %d sequences in the "
                "index. Continue anyway.\n",
...
char *bwa_set_rg(const char *s) {
    ...
    if (strstr(s, "\t") != NULL) {
        if (bwa_verbose >= 1)
            fprintf(stderr,
                    "[E::%s] the read group line contained literal <tab> "
