...
unsigned char nt_comp_table[256] = {
    ...
    'M', 'N', 'K', 'N', 'N', 'N', 'N', 'Y', 'S', 'A', 'N', 'B', 'W', 'X', 'R',
    'N', 'N', 'N', 'N', 'N', 'N', 'n', 't', 'v', 'g', 'h', 'n', 'n', 'c', 'd',
    'n', 'n', 'm', 'n', 'k', 'n', 'n', 'n', 'n', 'y', 's', 'a', 'n', 'b', 'w',
    'x', 'r', 'n', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N',
    'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N',
    'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N',
    'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N',
    'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N', 'N',
...
void bsw2_extend_left(const bsw2opt_t *opt, bwtsw2_t *b, uint8_t *_query,
                      int lq, uint8_t *pac, bwtint_t l_pac, uint8_t *_mem) {
    int i;
    bwtint_t k;
    ...
    for (i = 0; i < b->n; ++i) {
        ...
        if (lt > p->k)
            lt = p->k;
        for (k = p->k - 1, j = 0; k > 0 && j < lt; --k)
            target[j++] = pac[k >> 2] >> (~k & 3) * 2 & 0x3;
        lt = j;
        score =
            ksw_extend(p->beg, &query[lq - p->beg], lt, target, 5, mat, opt->q,
                       opt->r, opt->bw, 0, -1, p->G, &qle, &tle, 0, 0, 0);
...
void bsw2_extend_rght(const bsw2opt_t *opt, bwtsw2_t *b, uint8_t *query, int lq,
                      uint8_t *pac, bwtint_t l_pac, uint8_t *_mem) {
    int i;
    bwtint_t k;
    uint8_t *target;
    int8_t mat[25];
    ...
    for (i = 0; i < b->n; ++i) {
        ...
        lt = j;
        score =
            ksw_extend(lq - p->beg, &query[p->beg], lt, target, 5, mat, opt->q,
                       opt->r, opt->bw, 0, -1, 1, &qle, &tle, 0, 0, 0) -
            1;
        //		if (score < p->G) fprintf(stderr, "[bsw2_extend_hits] %d <
        //%d\n", score, p->G);
        if (score >= p->G) {
            p->G = score;
            p->len = tle;
...
static void gen_cigar(const bsw2opt_t *opt, int lq, uint8_t *seq[2],
                      int64_t l_pac, const uint8_t *pac, bwtsw2_t *b,
                      const char *name) {
    int i;
    int8_t mat[25];
    ...
    for (i = 0; i < b->n; ++i) {
        ...
        q->cigar = bwa_gen_cigar(mat, opt->q, opt->r, opt->bw, l_pac, pac,
                                 end - beg, query, p->k, p->k + p->len, &score,
                                 &q->n_cigar, &q->nm);
#if 0
...
void bsw2_debug_hits(const bwtsw2_t *b) {
    ...
    for (i = 0; i < b->n; ++i) {
        bsw2hit_t *p = b->hits + i;
        if (p->G > 0)
            printf("G=%d, G2=%d, len=%d, [%d,%d), k=%lu, l=%lu, #seeds=%d, "
                   "is_rev=%d\n",
                   p->G, p->G2, p->len, p->beg, p->end, (long)p->k, (long)p->l,
                   p->n_seeds, p->is_rev);
    }
}
...
static bwtsw2_t *bsw2_aln1_core(const bsw2opt_t *opt, const bntseq_t *bns,
                                uint8_t *pac, const bwt_t *target, int l,
                                uint8_t *seq[2], bsw2global_t *pool) {
    extern void bsw2_chain_filter(const bsw2opt_t *opt, int len,
                                  bwtsw2_t *b[2]);
    ...
    for (k = 0; k < 2; ++k) {
        bsw2_extend_left(opt, bb[k][1], seq[k], l, pac, bns->l_pac,
                         pool->aln_mem);
        merge_hits(bb[k], l, 0); // bb[k][1] is merged to bb[k][0] here
        bsw2_resolve_duphits(0, 0, bb[k][0], 0);
        bsw2_extend_rght(opt, bb[k][0], seq[k], l, pac, bns->l_pac,
                         pool->aln_mem);
        bsw2_resolve_duphits(0, 0, bb[k][0], 0);
        b[k] = bb[k][0];
        free(bb[k]);
...
static void flag_fr(bwtsw2_t *b[2]) {
    ...
    for (i = 0; i < b[0]->n; ++i) {
        ...
        for (j = 0; j < b[1]->n; ++j) {
            ...
            if (q->beg == p->beg && q->end == p->end && q->k == p->k &&
                q->len == p->len && q->G == p->G) {
                q->flag |= 0x30000;
                p->flag |= 0x30000;
                break;
        ...
        for (i = j = 0; i < n_cigar; ++i) {
            ...
            if (op == 4 || op == 5 || op == 1) { // ins or clipping
                ...
                if (x + ln >= refl && nc == 0) {
                    ...
                    cn[j++] =
                        (uint32_t)(lq - y - (refl - x)) << 4 | 4; // write S
                    nc = j;
                    mq[0] += refl - x;
...
static void write_aux(const bsw2opt_t *opt, const bntseq_t *bns, int qlen,
                      uint8_t *seq[2], const uint8_t *pac, bwtsw2_t *b,
                      const char *name) {
    int i;
    // allocate for b->aux
    if (b->n << 1 < b->max) {
    ...
    for (i = 0; i < b->n; ++i) {
        ...
        if (p->l == 0) { // unique hit
            ...
            if (p->n_seeds < 2)
                c *= .2;
            q->qual = (int)(c * (p->G - subo) * (250.0 / p->G + 0.03 / opt->a) +
                            .499);
            if (q->qual > 250)
                q->qual = 250;
            if (q->qual < 0)
...
static void update_mate_aux(bwtsw2_t *b, const bwtsw2_t *m) {
    ...
    if (b->n == 1 && m->n == 1) {
        ...
        if (p->flag &
            BSW2_FLAG_MATESW) { // this alignment is found by Smith-Waterman
            if (!(p->flag & BSW2_FLAG_TANDEM) && b->aux[0].pqual < 20)
                b->aux[0].pqual = 20;
            if (b->aux[0].pqual >= m->aux[0].qual)
                b->aux[0].pqual = m->aux[0].qual;
        } else if ((p->flag & 2) &&
                   !(m->hits[0].flag & BSW2_FLAG_MATESW)) { // properly paired
            if (!(p->flag &
                  BSW2_FLAG_TANDEM)) { // pqual is bounded by
                                       // [b->aux[0].qual,m->aux[0].qual]
                b->aux[0].pqual += 20;
                if (b->aux[0].pqual > m->aux[0].qual)
                    b->aux[0].pqual = m->aux[0].qual;
...
static void print_hits(const bntseq_t *bns, const bsw2opt_t *opt,
                       bsw2seq1_t *ks, bwtsw2_t *b, int is_pe,
                       bwtsw2_t *bmate) {
    int i, k;
    kstring_t str;
    ...
    for (i = 0; b && i < b->n; ++i) {
        ...
        if (q->cigar == 0)
            q->flag |= 0x4;
        ksprintf(&str, "%s\t%d", ks->name,
                 q->flag | (opt->multi_2nd && i ? 0x100 : 0));
        ksprintf(&str, "\t%s\t%ld", q->chr >= 0 ? bns->anns[q->chr].name : "*",
                 (long)q->pos + 1);
        if (p->l == 0 && q->cigar) { // not a repetitive hit
            ksprintf(&str, "\t%d\t", q->pqual);
            for (k = 0; k < q->n_cigar; ++k)
                ksprintf(&str, "%d%c", q->cigar[k] >> 4,
                         (opt->hard_clip ? "MIDNHHP"
                                         : "MIDNSHP")[q->cigar[k] & 0xf]);
        } else
            ksprintf(&str, "\t0\t*");
        if (!is_pe)
            kputs("\t*\t0\t0\t", &str);
        ...
        if (ks->qual) {
            ...
            kputs("\t*", &str);
        // print optional tags
        ksprintf(&str, "\tAS:i:%d\tXS:i:%d\tXF:i:%d\tXE:i:%d\tNM:i:%d", p->G,
                 p->G2, p->flag >> 16, p->n_seeds, q->nm);
        if (q->nn)
...
static void update_opt(bsw2opt_t *dst, const bsw2opt_t *src, int qlen) {
    ...
    *dst = *src;
    ...
    // set band width: the query length sets a boundary on the maximum band
    // width
...
static void bsw2_aln_core(bsw2seq_t *_seq, const bsw2opt_t *_opt,
    ...
    for (x = 0; x < _seq->n; ++x) {
        ...
        if (pool->max_l <
            l) { // then enlarge working space for aln_extend_core()
...
static void process_seqs(bsw2seq_t *_seq, const bsw2opt_t *opt,
                         const bntseq_t *bns, uint8_t *pac, const bwt_t *target,
                         int is_pe) {
    int i;
    is_pe = is_pe ? 1 : 0;
...
void bsw2_aln(const bsw2opt_t *opt, const bntseq_t *bns, bwt_t *const target,
