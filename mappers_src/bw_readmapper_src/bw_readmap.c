...
static void read_callback(const char *read_name, const char *read,
                          const char *quality, void *callback_data) {
    ...
    char match_buffer[n + 1];
    match_buffer[n] = '\0'; // FIXME: debug
    ...
    for (size_t seq_no = 0; seq_no < no_records; seq_no++) {
        ...
        search(read_name, read, strlen(read), quality, ref_name, 0,
               sa->length - 1, info->edit_dist, cigar, cigar_buffer + n - 1, sa,
               info->samfile);
    }
}
...
int main(int argc, char *argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "hpvd:", longopts, NULL)) != -1) {
        ...
        case 'h':
            ...
            printf(
                "\t-d | --distance:\t Maximum edit distance for the search.\n");
            printf("\t-p | --preprocess:\t Preprocess a reference genome.\n");
            printf("\t-v | --validate:\t Checks that preprocessed data matches "
                   "the computed tables reference genome.\n");
            printf("\n\n");
            return EXIT_SUCCESS;
    ...
    if (preprocess) {
        ...
        struct suffix_array_records *loaded_sa_records =
            empty_suffix_array_records();

        if (0 != read_suffix_array_records(loaded_sa_records, fasta_records,
                                           argv[0])) {
        ...
        for (size_t i = 0; i < no_records; ++i) {
            ...
            struct suffix_array *loaded_sa =
                loaded_sa_records->suffix_arrays[i];
            if (built_sa->length != loaded_sa->length) {
                fprintf(stderr,
                        "The suffix arrays disagree on length. %lu != %lu\n",
            ...
            for (size_t j = 0; j < built_sa->length; j++) {
                if (built_sa->array[j] != loaded_sa->array[j]) {
                    fprintf(stderr,
                            "The suffix arrays disagree on index %lu : %lu != "
                            "%lu\n",
